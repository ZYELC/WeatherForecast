...
#define kCityColor  COLOR_RGB(245, 245, 245)
...
- (void)createBottomBtn{
    ...
    // 添加
    UIButton *addCityBtn = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 50, 5, 30, 30)];
    addCityBtn.tintColor = [UIColor grayColor];
    UIImage *addImage = [[UIImage imageNamed:@"sb_add"]imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
    [addCityBtn setBackgroundImage:addImage forState:UIControlStateNormal];
    addCityBtn.tintColor = [UIColor grayColor];
    [addCityBtn addTarget:self action:@selector(pushCitysViewController) forControlEvents:UIControlEventTouchUpInside];
    [self.bottomView addSubview:addCityBtn];
    
    
    UIButton *editBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 5, 30, 30)];
    [editBtn setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
    editBtn.titleLabel.font = [UIFont systemFontOfSize:30];
    [editBtn setTitle:@"E" forState:UIControlStateNormal];
    ...
    editBtn.center = CGPointMake(SCREEN_WIDTH / 2, self.bottomView.height / 2);
...
// 点击编辑按钮
- (void)editButtonClick:(UIButton *)item
{
    if ([item.currentTitle isEqualToString:@"E"]) {
        [item setTitle:@"D" forState:UIControlStateNormal];
    }
    else
    {
        [item setTitle:@"E" forState:UIControlStateNormal];
    }
...
- (void)createTableView
{
    ...
    _tableView.backgroundColor = kCityColor;
...
- (void)headerViewBtnClick{
    ...
    {
        [ZDMView showAlertViewWithMessage:@"请点击开关打开定位" target:self];
    }
}
#pragma mark 改变定位开关
...
//            [ZDMView showHUDWithText:@"正在定位..." toView:self.view];
        }
        else{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.backgroundColor = kCityColor;
    return cell;
}
#pragma mark cell的高度
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray<CLLocation *> *)locations
{
    ...
    [ZDMView showHUDWithText:@"正在定位..." toView:_tableView];
        ...
        [self.geoceder reverseGeocodeLocation:location completionHandler:^(NSArray<CLPlacemark *> * _Nullable placemarks, NSError * _Nullable error) {
            ...
            if (error || placemarks.count == 0) {
                [UIAlertController showAlertViewWithMessage:@"定位失败" target:self handler:^{
                    ...
                    // 隐藏加载视图
                    [ZDMView hideAllHUDFromView:_tableView];
                }];
            }
            else
            {
                ...
                if(placemark.locality != nil){
                    // 搜索该城市
                    [[AFHTTPRequestOperationManager manager] GET:kSearchCity parameters:@{@"p0" : placemark.locality} success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
                        // 隐藏加载视图
                        [ZDMView hideAllHUDFromView:_tableView];
                        ...
                        _locationSwitch.on = NO;
                        [ZDMView showAlertViewWithMessage:@"定位失败，请检查网络" target:self];
                        
                        NSLog(@"error -- %@",error.localizedDescription);
                    }];
                }else
                {
                    [UIAlertController showAlertViewWithMessage:@"定位失败,请确认设备的语言为简体中文" target:self handler:^{
                        _locationSwitch.on = NO;
                        // 隐藏加载视图
                        [ZDMView hideAllHUDFromView:_tableView];
                    }];
...
- (void)addLocationCityToCityArrayWithCityModel:(CityModel *)model
{
    ...
    if (!isExist) {
        [ZDMTool sendAFNetworkGet:kWeather parameters:@{@"cityIds" : model.code} success:^(id responseObject) {
