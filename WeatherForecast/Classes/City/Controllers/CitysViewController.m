...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    UIButton *btn = [ZDMView createButtonWithFrame:CGRectMake(10, 0, SCREEN_WIDTH, 39) title:title target:self action:@selector(changeExpandSection:) backgroundColor:COLOR_WHITECOLOR];
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
    ...
    {
        //显示加载视图
        [ZDMView showHUDWithText:@"正在搜索..." toView:self.view];
        // 从网络加载数据
        [[AFHTTPRequestOperationManager manager] GET:kSearchCity parameters:@{@"p0" : searchBar.text} success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
            // 隐藏加载视图
            [ZDMView hideHUDFromView:self.view];
